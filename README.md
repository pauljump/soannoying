...
The latest Codex screening pass produced `50` stronger final annoyance records from the private corpus. They passed a source audit: every record points to real corpus evidence, and every record has multiple distinct source URLs. They are the current final selection on the [public page](https://pauljump.github.io/soannoying/). “Final annoyance” means we decided the annoyance belongs in this release. It does not mean the annoyance is solved, universally measured, or already a reviewed problem record.

The raw leads and screened candidate work stay in the repo, but they are not homepage cards. The public page also has a separate city layer: the first release has `31` final annoyance records from San Francisco. City records are not used to filter or redefine the general list.
